...
async def test_send_donate_mock():
    ...
    with (
        ...
        patch(
            "neonpay.payments.UpdateBotPrecheckoutQuery"
        ) as mock_update_bot_precheckout_query,
        patch(
            "neonpay.payments.MessageActionPaymentSentMe"
        ) as mock_message_action_payment_sent_me,
        patch("neonpay.payments.SendMedia") as mock_send_media,
        patch(
            "neonpay.payments.SetBotPrecheckoutResults"
        ) as mock_set_bot_precheckout_results,
    ):
