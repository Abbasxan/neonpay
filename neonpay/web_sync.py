...
class SyncWebHandler:
    ...
    def __init__(self, neonpay_instance: Any, webhook_secret: Optional[str] = None) -> None:
                    ...
                    for promo_data_item in promo_data:
                        try:
                            ...
                            promo_system.create_promo_code(
                                ...
                                discount_type=DiscountType(promo_data_item["discount_type"]),
                            ...
                            logger.error(f"Failed to apply promo code {promo_data_item.get('code')}: {e}")
            ...
            if action == "sync":
                ...
                if hasattr(self.neonpay, 'templates') and self.neonpay.templates:
                    ...
                    for template in templates:
                        templates_data[template.name] = template_manager.export_template(template, "json")
        ...
        try:
            status_data = {
                ...
                "local_data": {
                    ...
                    "promo_codes": len(self.neonpay.promotions.list_promo_codes()) if hasattr(self.neonpay, 'promotions') and self.neonpay.promotions else 0,
                    "templates": len(self.neonpay.templates.list_templates()) if hasattr(self.neonpay, 'templates') and self.neonpay.templates else 0
                },
                "timestamp": time.time()
            }
...
def create_sync_app(neonpay_instance: Any, webhook_secret: Optional[str] = None) -> web.Application:
...
async def run_sync_server(neonpay_instance: Any, 
                         host: str = "0.0.0.0", 
                         port: int = 8080,
    ...
    logger.info(f"Sync server started successfully")
