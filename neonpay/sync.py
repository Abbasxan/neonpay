...
class ConflictResolver:
    ...
    def __init__(self, strategy: ConflictResolution = ConflictResolution.ASK_USER) -> None:
    ...
    def _merge_data(self, source: Dict[str, Any], target: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _ask_user_resolution(self, conflict: SyncConflict) -> Dict[str, Any]:
        ...
        logger.warning(f"Conflict resolution needed for {conflict.item_type}:{conflict.item_id}")
...
class SyncManager:
    ...
    async def sync_promo_codes(self) -> Dict[str, Any]:
        ...
        if target_data and hasattr(self.neonpay, 'promotions') and self.neonpay.promotions:
                ...
                if existing_promo:
                    ...
                    if self._has_conflict(existing_promo, promo_data):
                        conflict = SyncConflict(
                            ...
                            item_id=code,
                            source_data=self.serializer.serialize_promo_codes([existing_promo])[0],
                        ...
                        if resolved_data:
                            ...
                            promo_system.create_promo_code(
                                ...
                                discount_type=DiscountType(resolved_data["discount_type"]),
                                discount_value=resolved_data["discount_value"],
                                **{k: v for k, v in resolved_data.items() 
    ...
    async def sync_templates(self) -> Dict[str, Any]:
        ...
        if hasattr(self.neonpay, 'templates') and self.neonpay.templates:
            ...
            for template in templates:
                local_templates[template.name] = template_manager.export_template(template, "json")
        ...
        if target_data and hasattr(self.neonpay, 'templates') and self.neonpay.templates:
            template_manager = self.neonpay.templates
            ...
            for template_name, template_data in target_data.items():
                ...
                if existing_template:
                    # Check for conflicts
                    existing_json = template_manager.export_template(existing_template, "json")
    ...
    def get_sync_stats(self) -> Dict[str, Any]:
        ...
        successful_syncs = len([s for s in self._sync_history if s.status == SyncStatus.COMPLETED])
        failed_syncs = len([s for s in self._sync_history if s.status == SyncStatus.FAILED])
        ...
        return {
            ...
            "last_sync": self._sync_history[-1].start_time if self._sync_history else None,
