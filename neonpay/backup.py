...
class BackupManager:
    ...
    def _load_existing_backups(self) -> None:
        ...
        backup_info_file = os.path.join(self.config.backup_directory, "backup_info.json")
        ...
        try:
            ...
            if self.config.compression:
                ...
                with zipfile.ZipFile(backup_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                    zipf.writestr("backup_data.json", json.dumps(data, indent=2, ensure_ascii=False))
            else:
    ...
    async def restore_backup(self, backup_id: str) -> bool:
        ...
        try:
            ...
            if self.config.compression and backup_info.file_path.endswith('.zip'):
                ...
                async with aiofiles.open(backup_info.file_path, 'r', encoding='utf-8') as f:
                    data = json.loads(await f.read())
            
            # Restore payment stages
            if "payment_data" in data and "payment_stages" in data["payment_data"]:
                for stage_id, stage_data in data["payment_data"]["payment_stages"].items():
...
class SyncManager:
    ...
    async def sync_with_bot(self, target_bot_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            target_neonpay = create_neonpay(bot_instance=None)  # This would need proper bot instance
    ...
    async def import_data(self, data: str, format_type: str = "json") -> bool:
        ...
        try:
            ...
            if "payment_data" in imported_data and "payment_stages" in imported_data["payment_data"]:
                for stage_id, stage_data in imported_data["payment_data"]["payment_stages"].items():
                    from .core import PaymentStage
                    stage = PaymentStage(
                        title=stage_data["title"],
                        description=stage_data["description"],
