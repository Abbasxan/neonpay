        ...
        try:
            ...
            if not bot_id or not events:
                return web.json_response({
                    "status": "error",
                    "message": "Missing bot_id or events"
                }, status=400)
    ...
    async def handle_analytics_query(self, request: Request) -> Response:
        ...
        try:
            ...
            if bot_id:
                ...
                analytics = self.multi_bot_analytics.get_bot_analytics(bot_id, days=days)
                if analytics:
                    data = {
...
def create_analytics_app(multi_bot_analytics: Any,
                        event_collector: Any,
                        webhook_secret: Optional[str] = None) -> web.Application:
...
async def run_analytics_server(multi_bot_analytics: Any,
                              event_collector: Any,
                              host: str = "0.0.0.0",
                              port: int = 8081,
                              webhook_secret: Optional[str] = None) -> None:
