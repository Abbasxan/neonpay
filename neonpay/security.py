...
class RateLimiter:
    ...
    def set_limit(self, action_type: ActionType, max_requests: int, time_window: int) -> None:
        ...
        logger.info(f"Set rate limit for {action_type.value}: {max_requests}/{time_window}s")
    
    def is_allowed(self, user_id: int, action_type: ActionType) -> Tuple[bool, Optional[int]]:
        """
        ...
        while (self._requests[key] and 
               current_time - self._requests[key][0] > limit.time_window):
    ...
    def get_remaining_requests(self, user_id: int, action_type: ActionType) -> Optional[int]:
...
class SecurityManager:
    ...
    def set_rate_limit(self, action_type: ActionType, max_requests: int, time_window: int) -> None:
        """Set custom rate limit"""
        self._rate_limiter.set_limit(action_type, max_requests, time_window)
    
    def check_rate_limit(self, user_id: int, action_type: ActionType) -> Tuple[bool, Optional[int]]:
    ...
    def report_suspicious_activity(
        ...
        **metadata
        ...
        if (self._auto_block_enabled and 
            profile.risk_score >= self._max_risk_score and 
            not profile.trusted):
    ...
    def block_user(self, user_id: int, duration: Optional[int] = None) -> None:
        ...
        logger.warning(f"User {user_id} blocked" + (f" for {duration}s" if duration else " permanently"))
    ...
    def detect_payment_fraud(
        ...
        recent_events = [
            event for event in profile.suspicious_activities
            if (event.event_type == "payment_completion" and 
        ...
        if amount > 1000:  # More than 1000 Stars
            recent_high_payments = [
                event for event in profile.suspicious_activities
                if (event.event_type == "high_amount_payment" and 
    ...
    def get_security_stats(self) -> Dict[str, Any]:
        ...
        blocked_users = sum(1 for profile in self._user_profiles.values() 
                          if profile.is_currently_blocked())
        high_risk_users = sum(1 for profile in self._user_profiles.values() 
                            if profile.risk_score > 50)
        trusted_users = sum(1 for profile in self._user_profiles.values() 
                          if profile.trusted)
