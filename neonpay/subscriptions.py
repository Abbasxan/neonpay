...
class SubscriptionManager:
    ...
    def __init__(self, max_plans: int = 100, max_subscriptions: int = 10000):
        ...
        self._user_subscriptions: Dict[int, List[str]] = {}  # user_id -> subscription_ids
    ...
    def subscribe_user(
        ...
        if len(self._subscriptions) >= self._max_subscriptions:
            raise ValueError(f"Maximum number of subscriptions ({self._max_subscriptions}) reached")
        ...
        if plan.max_subscribers:
            active_subs = sum(1 for sub in self._subscriptions.values() 
        ...
        subscription = Subscription(
            ...
            user_id=user_id,
            plan=plan,
            status=SubscriptionStatus.ACTIVE if plan.trial_days == 0 else SubscriptionStatus.PENDING
        )
    ...
    async def check_renewals(self) -> List[Subscription]:
        ...
        for subscription in self._subscriptions.values():
            if (subscription.is_active() and 
                subscription.next_billing_at and 
                current_time >= subscription.next_billing_at):
    ...
    async def check_expirations(self) -> List[Subscription]:
        ...
        for subscription in self._subscriptions.values():
            if subscription.is_expired() and subscription.status != SubscriptionStatus.EXPIRED:
