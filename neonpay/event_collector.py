    ...
    async def collect_events(self) -> List[Dict[str, Any]]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.webhook_url}/analytics/events") as response:
                    if response.status == 200:
                        ...
                        logger.info(f"Collected {len(events)} events from {self.bot_name}")
                        return events
                    else:
                        logger.warning(f"Failed to collect events from {self.bot_name}: {response.status}")
                        return []
        except Exception as e:
            logger.error(f"Error collecting events from {self.bot_name}: {e}")
            return []
            
    async def send_events_to_central(self, 
        ...
        for attempt in range(max_retries):
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(f"{central_url}/analytics/collect", json=payload) as response:
                        if response.status in [200, 201]:
                            logger.info(f"Sent {len(events)} events to central analytics from {self.bot_name}")
                            return True
                        else:
                            logger.warning(f"Failed to send events to central analytics: {response.status}")
                            
            except Exception as e:
                logger.error(f"Error sending events to central analytics (attempt {attempt + 1}): {e}")
...
class CentralEventCollector:
    ...
    async def _collect_from_all_bots(self) -> None:
        ...
        if tasks:
            ...
            total = len(results)
            logger.info(f"Event collection completed: {successful}/{total} bots successful")
        ...
        try:
            ...
            if events:
                ...
                success = await collector.send_events_to_central(
                    events, 
                    self.config.central_analytics_url,
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.central_analytics_url}/analytics/realtime",
                    json=event
                ) as response:
                    if response.status in [200, 201]:
                        logger.debug(f"Sent real-time event to central analytics")
                    else:
                        logger.warning(f"Failed to send real-time event: {response.status}")
        except Exception as e:
