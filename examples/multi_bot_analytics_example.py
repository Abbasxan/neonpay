...
from neonpay import (
    PaymentStage, MultiBotAnalyticsManager, MultiBotSyncManager, BotSyncConfig, SyncDirection,
    EventCollectorConfig, MultiBotEventCollector
)
...
class MultiBotAnalyticsDemo:
    ...
    async def setup_bots(self):
        ...
        self.event_collector.add_bot("store_bot", "Main Store Bot", "https://store-bot.example.com")
        self.event_collector.add_bot("support_bot", "Support Bot", "https://support-bot.example.com")
        self.event_collector.add_bot("analytics_bot", "Analytics Bot", "https://analytics-bot.example.com")
    ...
    async def simulate_events(self):
        ...
        for i in range(100):
            ...
            # Random event type
            event_types = [
                "user_started", "product_view", "product_click", 
                "payment_started", "payment_completed", "payment_failed"
            ]
            event_type = random.choice(event_types)
            
            # Random amount for payment events
            amount = random.randint(10, 50) if event_type == "payment_completed" else None
    ...
    async def demonstrate_analytics(self):
        ...
        if network_analytics:
            ...
            logger.info(f"  Network Conversion Rate: {network_analytics.network_conversion_rate:.1f}%")
            
            logger.info("\n🏆 **Top Performing Bots:**")
            for i, bot in enumerate(network_analytics.top_performing_bots[:3], 1):
                logger.info(f"  {i}. {bot['bot_name']}: {bot['revenue']} stars ({bot['transactions']} transactions)")
                
            logger.info("\n📈 **Top Products:**")
            for i, product in enumerate(network_analytics.top_products[:3], 1):
                logger.info(f"  {i}. {product['product_id']}: {product['revenue']} stars")
    ...
    async def demonstrate_export(self):
        ...
        # Export to JSON
        json_data = self.multi_analytics.export_network_analytics(format_type="json", days=1)
        if json_data:
            logger.info("✅ JSON export successful")
            logger.info(f"  Data size: {len(json_data)} characters")
            
        # Export to CSV
        csv_data = self.multi_analytics.export_network_analytics(format_type="csv", days=1)
        if csv_data:
    ...
    async def demonstrate_event_collection(self):
        ...
        logger.info(f"  Collection Interval: {stats['collector_stats']['collection_interval']} seconds")
        logger.info(f"  Real-time Enabled: {stats['collector_stats']['realtime_collection']['enabled']}")
