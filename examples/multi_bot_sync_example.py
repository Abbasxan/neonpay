...
from neonpay import (
    ...
    MultiBotSyncManager, BotSyncConfig, SyncDirection, ConflictResolution,
    SyncStatus
)
...
async def store_command(message: Message):
    ...
    if not stages:
        await message.answer("🛒 Store is empty. Products will appear after synchronization.")
        return
...
async def sync_stats_callback(callback: CallbackQuery):
    ...
    total_successful = sum(stats.get('successful_syncs', 0) for stats in all_stats.values())
...
async def show_store_callback(callback: CallbackQuery):
    ...
    if not stages:
        await callback.message.edit_text("🛒 Store is empty. Products will appear after synchronization.")
        await callback.answer()
        return
