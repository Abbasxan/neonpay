...
app = Client("neonpay_middleware_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)
...
async def donate_command(client: Client, message: Message):
    ...
    try:
        ...
        processed_stage = await middleware_manager.process_before_payment(stage, context)
        ...
        processed_result = await middleware_manager.process_after_payment(result, context)
        ...
        if should_continue:
            ...
            await message.reply_text("❌ Payment processing stopped due to critical error.")
