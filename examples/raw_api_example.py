...
async def handle_digital_purchase(result: PaymentResult):
    ...
    logger.info(f"💰 Digital purchase: {product_name} by user {user_id} for {amount} ⭐")
...
async def show_product_details(user_id: int, product_id: str):
    ...
    keyboard = {
        "inline_keyboard": [
            [{"text": f"💳 Buy Now ({product['price']} ⭐)", "callback_data": f"buy_{product_id}"}],
    ...
    try:
        if data == "show_catalog":
            ...
            if success:
                ...
                await answer_callback_query(callback_id, "❌ Failed to create payment", True)
