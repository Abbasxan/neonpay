...
async def show_store_options(callback_query: CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 Premium Access - 100⭐", callback_data="buy_premium_access")],
        [InlineKeyboardButton("🎨 Custom Theme - 200⭐", callback_data="buy_custom_theme")],
        [InlineKeyboardButton("⚡ Priority Support - 150⭐", callback_data="buy_priority_support")],
        [InlineKeyboardButton("🔙 Back", callback_data="back_main")]
    ])
...
async def show_user_stats(callback_query: CallbackQuery):
    ...
    if user_id in purchases:
        purchase_count = len([p for p in purchases.values() if p.get("product") != "donation"])
    else:
        purchase_count = 0
